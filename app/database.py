...
async def get_forecast_accuracy(days: int = 30) -> dict:
    """Forecast vs. actual data for the last N days.

    Joins forecast_log (predictions) with daily_solar (actuals).
    Returns per-day predictions for each source ('openmeteo', 'ml_solar', 'ml_load')
    plus aggregate accuracy metrics (MAE, RMSE, MAPE, bias).
    """
    db = await get_pool()
    cutoff = (_now_local() - timedelta(days=days)).strftime("%Y-%m-%d")
    ...
    cur = await db.execute(
        """SELECT fl.date, fl.source, fl.predicted_kwh, fl.predicted_load_kwh,
                  ds.solar_kwh, ds.total_output_kwh
           FROM forecast_log fl
           LEFT JOIN daily_solar ds ON ds.date = fl.date
           WHERE fl.date >= ? AND fl.date < ?
             AND fl.source IN ('openmeteo', 'ml_solar', 'ml_load')
    ...
    by_date: dict[str, dict] = {}
    for date, source, pred_solar, pred_load, actual_solar, actual_load in rows:
        if date not in by_date:
            by_date[date] = {
                "date": date,
                "actual_solar": actual_solar or 0,
                "actual_load": actual_load or 0,
            }
        if source == "ml_load":
            by_date[date]["pred_ml_load"] = pred_load or 0
        elif source == "openmeteo":
            by_date[date]["pred_openmeteo"] = pred_solar or 0
        elif source == "ml_solar":
            by_date[date]["pred_ml_solar"] = pred_solar or 0

    series = sorted(by_date.values(), key=lambda r: r["date"])

    def metrics(pred_key: str, actual_key: str) -> dict:
        pairs = [(r[pred_key], r[actual_key]) for r in series
                 if pred_key in r and r[actual_key] > 0.01]
        n = len(pairs)
        if n == 0:
            return {"n": 0, "mae": 0, "rmse": 0, "mape": 0, "bias": 0}
    ...
    return {
        ...
        "series": series,
        "metrics": {
            "openmeteo_solar": metrics("pred_openmeteo", "actual_solar"),
            "ml_solar":        metrics("pred_ml_solar", "actual_solar"),
            "ml_load":         metrics("pred_ml_load", "actual_load"),
        },
    }
